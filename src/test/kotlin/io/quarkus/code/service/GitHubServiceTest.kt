...
/**
 * To record:
 * 1. Get a "code" using https://github.com/login/oauth/authorize?client_id=e1177a88a6d9eec4bd16&scope=repo&state=lMqTg6m6A-wbzQsCv2qW8jW-y0Y (it's in the query param of the redirection)
...
internal class GitHubServiceTest {
    ...
    companion object {
        ...
        const val CODE = "08081fcf9c5db1fff06d"
    ...
    fun createAndPushRepository() {
        ...
        //when
        val result = gitHubService.createRepository(token.accessToken, REPO_TO_CREATE_NAME)
    ...
    fun `Should return true when the repository exists`() {
        val exists = gitHubService.repositoryExists(token.accessToken, AN_EXISTING_REPO)
        Assert.assertTrue(exists)
    }

    @Test
    fun `Should return false when the repository doesn't exist`() {
        val exists = gitHubService.repositoryExists(token.accessToken, "new-repo-name")
